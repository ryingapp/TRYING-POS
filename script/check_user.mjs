...
const LOCAL_SCRIPT = resolve("script/_check_user.cjs");
...
conn.on("ready", async () => {
  ...
  await upload(conn, LOCAL_SCRIPT, "/opt/trying/_check_user.cjs");
  const dbUrl = await exec(conn, "grep DATABASE_URL /opt/trying/.env | cut -d= -f2-");
  await exec(conn, `cd /opt/trying && DATABASE_URL="${dbUrl.trim()}" node _check_user.cjs`);
  conn.end();
});
