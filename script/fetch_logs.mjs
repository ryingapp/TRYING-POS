...
async function main() {
  ...
  console.log("Connecting to fetch logs...");
  ...
  // Check both PM2 logs and the file defined in ecosystem
  // The ecosystem.config.cjs defined error_file: "/var/log/trying-error.log"
  
  console.log("\n--- /var/log/trying-error.log (Tail 100) ---\n");
  const log1 = await exec(conn, "tail -n 100 /var/log/trying-error.log");
  console.log(log1.stdout);
  console.log(log1.stderr);

  console.log("\n--- PM2 Logs (tail) ---\n");
  const log2 = await exec(conn, "pm2 logs trying --lines 50 --nostream");
  console.log(log2.stdout);
  console.log(log2.stderr);
